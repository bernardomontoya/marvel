...
export default function ResourceList({
  ...
  const rowRenderer = ({ index, style }) => {
    ...
    const elementsIds = generateIndexesForRow(
      index,
      maxItemsPerRow,
      data.data.length
    );
    const currentRowElements = elementsIds.map(
      (elementIndex) => data.data[elementIndex]
    );
    return (
  ...
  function isItemLoaded(index) {
    ...
    const allItemsLoaded =
      generateIndexesForRow(index, maxItemsPerRow, data.data.length).length > 0;
