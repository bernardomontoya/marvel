...
const StyledChip = styled(Chip)`
    ...
    & svg {
        ...
        transform: rotate(${props => props.activechipprop === 'true' ? 180 : 0}deg) !important;
    }
`
...
function BuildOrderBy(props) {
    ...
    const orderByList = orderByElements.map((element) =>
        <Tooltip key={element.name} title={element.description} placement="top-start">
            <StyledChip
                size='small'
                color="primary"
                clickable
                icon={<MdExpandMore />}
                label={element.active && element.activeName !== '' ? element.activeName : element.name}
    ...
    return (
      <div className="flex flex-row items-center ml3">{orderByList}</div>
    );
}
function BuildSearchBy(props) {
    const searchByElements = props.searchBy;
    const handleCriteriaChange = props.handleCriteriaChange;
    const defaultCriteria = searchByElements.filter(element => element.active)[0].criteria;
    const searchByList = searchByElements.map((element) =>
        <Tooltip title={element.description} placement="top-start" key={element.criteria}>
            <FormControlLabel
                value={element.criteria}
                control={<Radio color="primary" onChange={()=> handleCriteriaChange(element.criteria, element.active, element.value)} />}
                label={element.title}
                labelPlacement="end"
            />
        </Tooltip>
    ...
    return (
        <SearchCriteria component="fieldset">
            <RadioGroup row aria-label="position" name="position" value={defaultCriteria}>
                {searchByList}
            </RadioGroup>
        </SearchCriteria>
    );
...
export default function ResourceFilters({ filters, setFilters }) {
    ...
    const activeDataType = filters.searchBy.filter(element => element.active === true)[0].type;
    // state
    ...
    const handleSearch = () => {
        ...
        const activeCriteria = searchByCopy.filter(element => element.active === true);
    ...
    const handleCriteriaChange = (criteria, active, value) => {
        ...
        searchByCopy.forEach(element => {
            element.active = (criteria === element.criteria && active === false ? true : false);
            element.value = (criteria === element.criteria && active === false ? value : '');
        });
        setFilters({ ...filters, searchBy: [...searchByCopy] });
    }
    const handleOrderBy = (criteria, active) => {
        ...
        if (orderByCopy.length > 0) {
            orderByCopy.map(element => {
                element.active = (element.criteria === criteria ? !active : element.active);
            });
        }
                ...
                <div className="w-100 ph3 ph6-ns flex flex-row justify-between">
                    <div className="flex flex-row w-auto w-100-ns overflow-x-auto">
                        <SearchContainer variant="outlined" className="w-auto">
                            ...
                            <OutlinedInput
                                id="search-resource"
                                type={activeDataType === 'string' ? 'text' : activeDataType === 'int' ? 'number' : 'text'}
                                value={searchString}
                        ...
                        {
                            searchBy.length > 0 ? <BuildSearchBy searchBy={searchBy} handleCriteriaChange={handleCriteriaChange} /> : null
                        }
                    </div>
                    {
                        orderBy.length > 0 ? <BuildOrderBy orderBy={orderBy} handleOrderBy={handleOrderBy} /> : null
                    }
            </div>
        </FiltersContainer>
    )
