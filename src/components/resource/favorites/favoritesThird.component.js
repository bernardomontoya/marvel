...
function BuildElements({
  ...
  const elementsList = data.map((element) => (
    <ResourceListElement
      key={resource}
      data={isDetail ? element : element.data}
  ...
  return (
    <div className="flex flex-row items-center overflow-x-auto flex-nowrap">
      {elementsList}
    </div>
  );
}

export default function FavoritesThird({
  favorites,
  resource,
  isDetail,
  total,
      ...
      {favorites.length > 0 ? (
        <div className="w-100 flex flex-column">
          <div className="pt3 pt5-ns pb3 pb4-ns flex flex-row justify-between">
            ...
            <h2 className="f2 f1-ns white">
              {isDetail ? total : favorites.length}
            </h2>
          </div>
          {favorites.length > 0 ? (
            <BuildElements
