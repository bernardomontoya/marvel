...
const Add = (id, resource, favorite, setFavorite, data) => {
  ...
  const currentElement = favorites.filter(
    (element) => element.id === id && element.resource === resource
  );
  // check element
...
export default function AddFavorite({ id, resource, data, isDetail }) {
  ...
  return (
    <AddFavoriteContainer
      className={
        "z-999 pointer flex flex-column " +
        (isDetail ? "absolute top-1 right-1" : "")
      }
      favorite={favorite}
      onClick={() => {
        Add(id, resource, favorite, setFavorite, data);
      }}
    >
