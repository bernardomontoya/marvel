...
export default function Third(props) {
    // React spring hook
    const [{ color }, setC] = useSpring(() => ({ color: "rgba(255, 255, 255, 0)" }));
    ...
    return(
        ...
        >
            <Link to={slug} onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc(x, y) })} className="flex flex-column items-center-ns justify-center overflow-x-auto" >
                <TitleContainer style={{ transform: springProps.xy.interpolate(transText)}}>
                    <animated.div className="f2 f1-l ttu z-999 tl flex flex-row flex-column-ns" style={{ color }}>
                        {
                            slug === 'characters' ?
                            <>
                                <Title>CHA</Title>
