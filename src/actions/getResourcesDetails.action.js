...
async function GetResourceDetails(
  ...
  try {
    ...
    const buildURLResourceOne = BuildURL(
      data,
      null,
      resource + "/" + id + "/" + resources[0],
      false
    );
    const buildURLResourceTwo = BuildURL(
      data,
      null,
      resource + "/" + id + "/" + resources[1],
      false
    );
    ...
    if (
      ...
      setData({
        ...data,
        dataGeneral: responseGeneralData,
        dataResourceOne: responseResourceOneData,
        dataResourceTwo: responseResourceTwoData,
        resource: resource,
      });
      setLoading(false);
    } else {
      setError({
        error: true,
        reason: "The element that you are looking for does not exists",
      });
      setLoading(false);
    }
  } catch (e) {
