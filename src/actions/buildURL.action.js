function BuildFilters(filters) {
  ...
  if (searchBy.length > 0) {
    ...
    const activeCriteria = searchByCopy.filter(
      (element) => element.active === true && element.value !== ""
    );
  ...
  if (orderBy.length > 0) {
    ...
    if (orderbyCopy.length > 0) {
      orderbyCopy.forEach((element, i) => {
        orderByString +=
          (element.active ? "-" + element.criteria : element.criteria) +
          (i + 1 === orderbyCopy.length ? "" : "%2C");
      });
    }
